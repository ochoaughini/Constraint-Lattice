...
def demonstrate_complete_system(iterations: int = 20) -> Dict[str, Any]:
    """
    ...
    agent.take_role("assistant")  # Start with a normal role

    # Generate reference embeddings for "normal" behavior
    reference_count = 10
    reference_embeddings = [
        agent.generate_embedding(f"Normal text {i}") for i in range(reference_count)
    ]
    ...
    for i in range(iterations):
        ...
        if detection_result["is_anomalous"]:
            print(
                f"[DETECTED] Detection result: ANOMALOUS (confidence: {detection_result['confidence']:.2f})"
            )
            print(
                f"   Methods triggered: {', '.join(detection_result['methods_triggered'])}"
            )
        else:
            print(
                f"[NORMAL] Detection result: NORMAL (confidence: {1 - detection_result['confidence']:.2f})"
            )
